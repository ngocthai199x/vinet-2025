...
export class CartService {
  ...
  addItemToCart(item: CartItem | Product, quantity = 1) {
    ...
    if (this.isProduct(item)) {
      item = this.mapProducToCartItem(item);
    }
  ...
  private addOrUpdateItem(
    ...
    //if product not exist in cart then add this product to cart
    if (index === -1) {
      item.quantity = quantity;
      items.push(item);
    } else {
      //else adjust the quatity only
      items[index].quantity += quantity;
    }
    return items;
  }
  private mapProducToCartItem(item: Product): CartItem {
    return {
      productId: item.id,
